 ...
 */
import twitter4s.{QueryResult, Twitter}
 ...
 */
trait SearchResourcesDsl extends Twitter4sDslBase with SearchResources {
  type ResourcesType = SearchResourcesImpl
  protected val twitter4sResources = new Twitter(twitter4jResources) with ResourcesType

  // bellow methods implements are a cliche.
  def search(query: Query): QueryResult = resources.search(query)
}
