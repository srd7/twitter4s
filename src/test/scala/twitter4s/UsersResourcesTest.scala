...
class UsersResourcesTest extends Specification with TwitterResourcesTestBase {
  ...
  "showUser" should {
    "call twitter4j showUser method by screen name" in {
      twitter.users.showUser(User.isSpecifiedBy("fake_user")).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).showUser("fake_user")
    }

    "call twitter4j showUser method by user id" in {
      twitter.users.showUser(User.isSpecifiedBy(1L)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).showUser(1L)
    }
    
    "throw IllegalArgumentException are not set both parameters" in {
      twitter.users.showUser(null) must throwA[IllegalArgumentException]
    }
  }

  "lookupUsers" should {
    "call twitter4j lookupUsers method by screen name array" in {
      twitter.users.lookupUsers(Users.areSpecifiedBy(fakeUsersScreenNameArray)).size must equalTo(1)
      there was one(mockedTwitter4j).lookupUsers(fakeUsersScreenNameArray)
    }

    "call twitter4j lookupUsers method by user id array" in {
      twitter.users.lookupUsers(Users.areSpecifiedBy(fakeUsersIdArray)).size must equalTo(1)
      there was one(mockedTwitter4j).lookupUsers(fakeUsersIdArray)
    }
    
    "throw IllegalArgumentException when specificUsers is null" in {
      twitter.users.lookupUsers(null) must throwA[IllegalArgumentException]
    }
  }
  
  "searchUser" should {
    "call twitter4j searchUser method" in {
      twitter.users.searchUsers("search user", 1).size must equalTo(1)
  ...
  "verifyCredentials" should {
    "call twitter4j verifyCredentials method" in {
      twitter.users.verifyCredentials.screenName must equalTo(FakeValuesUsedByMock.userName)
  ...
  "updateProfile" should {
    "call twitter4j updateProfile method" in {
      twitter.users.updateProfile("twt4s_id", "http://hoge.com", ":Location", "test profile").screenName must equalTo(FakeValuesUsedByMock.userName)
  ...
  "updateProfileColors" should {
    "call twitter4j updateProfileColors method" in {
      twitter.users.updateProfileColors("f00", "f0f", "0ff", "0f0", "f0f").screenName must equalTo(FakeValuesUsedByMock.userName)
  ...
  "getAccountSettings" should {
    "call twitter4j getAccountSettings method" in {
      twitter.users.getAccountSettings.accessLevel must equalTo(TwitterResponse.READ_WRITE)
  ...
  "updateAccountSettings" should {
    "call twitter4j updateAccountSettings method" in {
      twitter.users.updateAccountSettings(1, true, "23", "08", "Helsinki", "it").accessLevel must equalTo(TwitterResponse.READ_WRITE)
  ...
  "updateProfileImage" should {
    "call twitter4j updateProfileImage method by image stream" in {
      ...
      twitter.users.updateProfileImage(ImageResource.isAssigned(inputStream)).screenName must equalTo(FakeValuesUsedByMock.userName)
    ...
    "call twitter4j updateProfileImage method by image file" in {
      ...
      twitter.users.updateProfileImage(ImageResource.isAssigned(image)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).updateProfileImage(image)
    }
    
    "throws IllegalArgumentException with no image file and stream" in {
       twitter.users.updateProfileImage(null) must throwA[IllegalArgumentException]
    }
  }
  ...
  "updateProfileBackgroundImage" should {
    "call twitter4j updateProfileBackgroundImage method by image stream" in {
      val inputStream = new FileInputStream(getRandomlyChosenFile)
      twitter.users.updateProfileBackgroundImage(ImageResource.isAssigned(inputStream), true).screenName must equalTo(FakeValuesUsedByMock.userName)
    ...
    "call twitter4j updateProfileBackgroundImage method by image file" in {
      val image = getRandomlyChosenFile
      twitter.users.updateProfileBackgroundImage(ImageResource.isAssigned(image), false).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).updateProfileBackgroundImage(image, false)
    }
    
    "throws IllegalArgumentException with no image file and stream" in {
      twitter.users.updateProfileBackgroundImage(null, true) must throwA[IllegalArgumentException]
    }
  }
  
  "createBlock" should {
    "call twitter4j createBlock method by screen name" in {
      twitter.users.createBlock(User.isSpecifiedBy(FakeValuesUsedByMock.userName)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).createBlock(FakeValuesUsedByMock.userName)
    }

    "call twitter4j createBlock method by user id" in {
      twitter.users.createBlock(User.isSpecifiedBy(1L)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).createBlock(1L)
    }
    
    "throw exception when user specific info is set null" in {
      twitter.users.createBlock(null) must
  ...
  "destryoBlock" should {
    "call twitter4j destroyBlock method by screen name" in {
      twitter.users.destroyBlock(User.isSpecifiedBy(FakeValuesUsedByMock.userName)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).destroyBlock(FakeValuesUsedByMock.userName)
    }

    "call twitter4j destroyBlock method by user id" in {
      twitter.users.destroyBlock(User.isSpecifiedBy(2L)).screenName must equalTo(FakeValuesUsedByMock.userName)
      there was one(mockedTwitter4j).destroyBlock(2L)
    }
    
    "throw exception when user specific info is set null" in {
      twitter.users.destroyBlock(null) must
      throwA[IllegalArgumentException] 
    }
  }
  ...
  "getBlocksList" should {
    "call twitter4j getBlocksList method without parameter" in {
      val actual = twitter.users.getBlocksList()
    ...
    "call twitter4j getBlocksList method by cursor" in {
      val actual = twitter.users.getBlocksList(3L)
      actual.size must equalTo(50)
  ...
  "getBlocksIDs" should {
    "call twitter4j getBlocksIDs method without parameter" in {
      twitter.users.getBlocksIDs().accessLevel must equalTo(TwitterResponse.READ_WRITE_DIRECTMESSAGES)
      there was one(mockedTwitter4j).getBlocksIDs
    }

    "call twitter4j getBlocksIDs method by cursor" in {
      twitter.users.getBlocksIDs(4L).accessLevel must equalTo(TwitterResponse.READ_WRITE_DIRECTMESSAGES)
  ...
  "getContributees" should {
    "call twitter4j getContributees method by screen name" in {
      twitter.users.getContributees(User.isSpecifiedBy(FakeValuesUsedByMock.userName)).size must equalTo(1)
    ...
    "call twitter4j getContributees method by user id" in {
      twitter.users.getContributees(User.isSpecifiedBy(5L)).size must equalTo(1)
      there was one(mockedTwitter4j).getContributees(5L)
    }

    "throw exception when user specific info is set null" in {
      twitter.users.getContributees(null) must
      throwA[IllegalArgumentException]
    }
  }
  ...
  "getContributors" should {
    "call twitter4j getContirbutors method by screen name" in {
      twitter.users.getContributors(User.isSpecifiedBy(FakeValuesUsedByMock.userName)).size must equalTo(1)
    ...
    "call twitter4j getContributors method by user id" in {
      twitter.users.getContributors(User.isSpecifiedBy(6L)).size must equalTo(1)
      there was one(mockedTwitter4j).getContributors(6L)
    }

    "throw exception when user specific info is set null" in {
      twitter.users.getContributors(null) must
      throwA[IllegalArgumentException]
    }
  }
  ...
  "removeProfileBanner" should {
    "call twitter4j removeProfileBanner method" in {
      twitter.users.removeProfileBanner()
  ...
  "updateProfileBanner" should {
    "call twitter4j updateProfileBanner method by image stream" in {
      val inputStream = new FileInputStream(getRandomlyChosenFile)
      twitter.users.updateProfileBanner(ImageResource.isAssigned(inputStream))
    ...
    "call twitter4j updateProfileBanner method by image file" in {
      val image = getRandomlyChosenFile
      twitter.users.updateProfileBanner(ImageResource.isAssigned(image))
    ...
    "throw exception when image resources specific info is set null" in {
      twitter.users.updateProfileBanner(null) must
      throwA[IllegalArgumentException]
    }
  }
}